...
// =============================================================
// Modified: Volume limiting and tracking features added
// build:avc-7f3a92e1-2025 · origin:avacocloud/XHTTP-Installer
import { PassThrough, Readable, Transform } from "node:stream";
import { pipeline } from "node:stream/promises";
import { setDefaultResultOrder } from "node:dns";
import { createVolumeMiddleware } from "../lib/volume-limiter.js";
...
// ─────────────────────────────────────────────
const volumeMiddleware = createVolumeMiddleware({
  dataDir: "/tmp/xhttp-volume",
});
const ENABLE_VOLUME_TRACKING = (process.env.ENABLE_VOLUME_TRACKING || "1").trim() !== "0";
const MONTHLY_LIMIT_MB = parseInt(process.env.MONTHLY_LIMIT_MB || "0", 10) || 0;
const DAILY_LIMIT_MB = parseInt(process.env.DAILY_LIMIT_MB || "0", 10) || 0;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    //  Volume Limit Check
    // ─────────────────────────────────────────────
    if (ENABLE_VOLUME_TRACKING && (MONTHLY_LIMIT_MB > 0 || DAILY_LIMIT_MB > 0)) {
      if (!volumeMiddleware.checkLimit(req, res)) {
        res.statusCode = 429;
        res.setHeader("retry-after", "86400");
        const usage = volumeMiddleware.getUsagePercent();
        return res.end(
          JSON.stringify({
            error: "volume_limit_exceeded",
            daily_percent: usage.daily,
            monthly_percent: usage.monthly,
            message: `Volume limit exceeded. Reset in: ${usage.daily >= 100 ? "24 hours" : "next month"}`,
          })
        );
      }
    }
    ...
    try {
      ...
      //  Record Volume Usage
      // ─────────────────────────────────────────────
      if (ENABLE_VOLUME_TRACKING && upstream.headers) {
        const contentLength = upstream.headers.get("content-length");
        const downloadBytes = contentLength ? parseInt(contentLength, 10) : 0;
        const uploadBytes = req.headers["content-length"]
          ? parseInt(req.headers["content-length"], 10)
          : 0;

        const usageResult = volumeMiddleware.recordUsage(uploadBytes, downloadBytes);
        if (usageResult && !usageResult.allowed) {
          console.warn(`[VolumeTracker] Limit may be exceeded: ${usageResult.reason}`);
        }
      }
