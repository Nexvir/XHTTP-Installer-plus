...
- افزوده شدن ماژول مدیریت حجم (Volume Limiter) برای پیگیری و اعمال محدودیت‌های روزانه/ماهیانه
- ثبت و نگهداری آمار مصرف (روزانه، ماهانه، کل) در فایل آمار محلی
- ارسال سربرگ‌های مصرف (`X-Volume-*`) در پاسخ‌ها برای مشاهدهٔ سریع از سمت CDN
- ادغام گزینهٔ نصب پنل مدیریتی `X-UI` به‌صورت اختیاری برای مشاهده و مدیریت آمار
- اسکریپت‌های کمکی (`volume-helper.sh`, `setup-xui.sh`) جهت مدیریت ساده‌تر محدودیت‌ها و داشبورد
- افزودن مستندات جامع: راهنمای مدیریت حجم، راهنمای به‌روزرسانی و فایل‌های تطابق با مجوز (ATTRIBUTION, GPL3_COMPLIANCE)
- سازگاری با Vercel و Netlify؛ نسخهٔ Edge Function برای Netlify اضافه شد

---

لطفاً هنگام بازنشر یا توزیع مجدد این فورک، موارد زیر را حفظ شود:

- فایل `LICENSE` (GNU GPL-3.0) بدون تغییر
- نوتیس کپی‌رایت اصلی: Copyright © 2025 @avaco_cloud
- لینک به مخزن اصلی: https://github.com/avacocloud/XHTTP-Installer
- اشاره به نگهدارندهٔ این فورک: https://github.com/Nexvir

برای نصب سریع این فورک روی یک سرور Ubuntu به‌صورت روت، از دستور بالا استفاده کنید.
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/Nexvir/XHTTP-Installer-plus/main/Deploy-Ubuntu.sh)
```
...
**با git (دستی):**
```bash
git clone https://github.com/Nexvir/XHTTP-Installer-plus.git /root/XHTTP-Installer
...
**با zip (آفلاین):**
```bash
scp XHTTP-Installer-plus.zip root@SERVER_IP:/root/
ssh root@SERVER_IP
cd /root && unzip XHTTP-Installer-plus.zip && sudo bash Deploy-Ubuntu.sh
```
...
Fork maintained by [Nexvir](https://github.com/Nexvir) — https://github.com/Nexvir/XHTTP-Installer-plus
